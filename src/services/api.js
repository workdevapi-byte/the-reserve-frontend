...
const api = axios.create({
  baseURL: import.meta.env.VITE_API_URL || 'https://reserve-backend.vercel.app/api',
...
export const investmentsApi = {
  ...
  listCategories: () => api.get('/investments/categories'),
  createCategory: (data) => api.post('/investments/categories', data),
  history: () => api.get('/investments/history'),
};

export const allocationsApi = {
  get: (bankId) => api.get(`/allocations/${bankId}`),
  update: (bankId, allocations) => api.post(`/allocations/${bankId}`, { allocations }),
};
